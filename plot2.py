...
#collect employment ratios for the year 2014
sql_employment_ratios='''
   ...
   FROM female_employment_vs_population f   WHERE f.year =2014
'''
...
for row_num in xrange(len(ratio_split)):    
    ...
    df['percentage_employment_to_population_ratio'].plot(kind='bar', ax=axes,legend=True, title='females with respect to working population for the year : 2014 - figure '+str(row_num) , position=0,width=0.25)
    df['percentage_working_age_of_population'].plot(kind='bar', color='red',legend=True, ax=axes, title='females with respect to working population for the year : 2014 - figure '+str(row_num) , position=1,width=0.25)
    #axes.legend( (rects1[0], rects2[0], rects3[0]), ('y', 'z', 'k') )
    for p in axes.patches:
	axes.annotate(str(p.get_height()), (p.get_x() * 1.005, p.get_height() * 1.005))

    plt.savefig('figures/working_population_vs_females/working_population_2014_'+str(row_num)+'.png', bbox_inches='tight')
